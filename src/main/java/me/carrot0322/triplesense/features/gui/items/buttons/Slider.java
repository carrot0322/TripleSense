...
import me.carrot0322.triplesense.features.gui.Component;
...
public class Slider
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.rect(context.getMatrices(), this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? TripleSense.colorManager.getColorWithAlpha(TripleSense.moduleManager.getModuleByClass(me.carrot0322.triplesense.features.modules.client.ClickGui.class).hoverAlpha.getValue()) : TripleSense.colorManager.getColorWithAlpha(TripleSense.moduleManager.getModuleByClass(me.carrot0322.triplesense.features.modules.client.ClickGui.class).alpha.getValue()));
