...
import me.carrot0322.triplesense.features.gui.items.buttons.ModuleButton;
...
public class ClickGui extends Screen {
    ...
    public ClickGui() {
        super(Text.literal("TripleSense"));
        setInstance();
        load();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : TripleSense.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 105, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    TripleSense.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
