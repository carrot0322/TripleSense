...
import me.carrot0322.triplesense.mixin.IClientPlayerEntity;
...
public class PlayerManager {
    ...
    public void onSyncWithServer(PacketEvent.@NotNull Send event) {
        ...
        if (event.getPacket() instanceof UpdateSelectedSlotC2SPacket slot) {
            if (serverSideSlot == slot.getSelectedSlot() && !(TripleSense.moduleManager.isModuleEnabled("NoSlow") /*&& ModuleManager.noSlow.mode.getValue() == NoSlow.Mode.StrictNCP*/)) {
                event.cancel();
