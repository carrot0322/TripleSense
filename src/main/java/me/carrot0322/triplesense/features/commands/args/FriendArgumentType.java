...
public class FriendArgumentType implements ArgumentType<String> {
    private static final List<String> EXAMPLES = TripleSense.friendManager.getFriends().stream().limit(5).toList();
    ...
    public String parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (!TripleSense.friendManager.isFriend(friend)) throw new DynamicCommandExceptionType(
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(TripleSense.friendManager.getFriends(), builder);
    }
